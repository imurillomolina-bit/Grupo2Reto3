...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <!-- Parametros de entrada para el detalle de equipo. -->
    ...
    <xsl:template match="/">
        ...
        <xsl:choose>
            ...
            <xsl:when test="string-length(normalize-space($equipoId)) = 0">
                <article class="panel-error">
                    <h2>Error</h2>
                    <p>No se indico el equipo solicitado.</p>
                    <p>
                        <a href="equipo.php?temporada_id={$temporadaId}">Volver al listado de equipos</a>
                    </p>
                </article>
            </xsl:when>

            <xsl:when test="not($equipoSeleccionado)">
                <article class="panel-error">
                    <h2>Error</h2>
            ...
            <xsl:otherwise>
                <article class="team-headline">
                    ...
                    <div>
                        ...
                        <p>
                            <a class="team-cta-link" href="partidos.php?equipo_id={$equipoSeleccionado/@id}&amp;temporada_id={$temporadaId}">Ver partidos de este equipo</a>
                        </p>
                        <p>
                            <a class="team-cta-link" href="equipo.php?temporada_id={$temporadaId}">Volver al listado</a>
                        </p>
                    </div>
                </article>
